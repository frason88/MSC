...
data_path = 'msc_final_data_v1.csv'
data = pd.read_csv(data_path)


print("DataFrame columns:", data.columns)

text_column_name = 'tutor_comment'
...
def preprocess(text):
    ...
    text = re.sub(r"n't", ' not', text)  # Handle common contractions
    text = re.sub(r'\b\w{1,2}\b', '', text)  # Remove words with 1 or 2 characters
    text = re.sub(r'\s+', ' ', text)  # Remove extra spaces
    return text.strip()

# Tokenize stopwords after preprocessing
def tokenize_stopwords(stopwords):
    tokenized_stopwords = set()
    for word in stopwords:
        preprocessed_word = preprocess(word)
        if preprocessed_word:
            tokens = preprocessed_word.split()
            tokenized_stopwords.update(tokens)
    return tokenized_stopwords

# Apply preprocessing to the stopwords
preprocessed_stopwords = tokenize_stopwords(custom_stopwords)

# Apply preprocessing to the text data
data['processed_text'] = data[text_column_name].apply(preprocess)

# Vectorize the data using TF-IDF
vectorizer = TfidfVectorizer(stop_words=list(preprocessed_stopwords))
...
plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c='blue', label='Data points')
...
#  LDA
lda_model = LatentDirichletAllocation(n_components=10, max_iter=10, random_state=0, learning_method='online')
...
top_words = get_top_words(lda_model, tf_feature_names, 10)

# Computing Coherence Score
id2word = gensim.corpora.Dictionary([doc.split() for doc in data['processed_text']])
corpus = [id2word.doc2bow(doc.split()) for doc in data['processed_text']]
coherence_model_lda = CoherenceModel(topics=top_words, texts=[doc.split() for doc in data['processed_text']], dictionary=id2word, coherence='c_v')
